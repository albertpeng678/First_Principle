...
#### Layer 1.1 — Baseline 截圖規範（Bundle 0a 補洞）
- **3-viewport 並排 frame 的處理：** mockup 為 vp-grid 排版（1920px 真實 viewport），但 baseline 必須切 frame 各自截 360 / 768 / 1280 三種 production viewport 的真實環境 — 用 `element.screenshot({ clip: ... })` 對單一 frame 取，再 resize 到 `{viewport}` 物理像素截。**不**直接用 1920 全 mockup PNG 跟 production diff（環境不同必爆 threshold）
- **動畫元素必凍 frame 0：** 截圖前注入 CSS `* { animation-play-state: paused !important; transition: none !important; }`；spinner / pulse dots / shimmer skeleton / streaming chat 全停在 frame 0；Layer 2 production diff 同樣套用
- **Hover / focus state：** 用 Playwright `.hover()` 觸發後等 250ms（過 transition）再截
- **Onboarding spotlight 容差：** spotlight ring 受 target 元素 box 影響，baseline ↔ production diff 對 spotlight 區域用 ±2px tolerance（`pixelmatch` 加 `aaColor` 寬鬆設定）
- **SVG radar / chart：** 要先等 `requestAnimationFrame × 2` 確保 Chrome paint 完整，避免半渲染 baseline
- **Web font：** 等 `document.fonts.ready` 才截（特別 Instrument Serif Italic）
...
- **無 magic link / OAuth**
- **無 token refresh** — JWT 過期就要重新登入（supabase 預設 1 hr）

### 1.5.1 Multi-tab + 401 mid-action 行為（CONTRACT-LOCKED — Bundle 0a 補洞）
**多 browser tab 並開：**
- AppState 不跨 tab 同步（無 BroadcastChannel）—— 各 tab 獨立 read localStorage
- 但 `localStorage` 儲存的 token / guestId 是 shared：tab A 登出 → token 從 storage 移除，tab B 不會即時知道，下一次 API call 才會 401
- **不主動偵測**其他 tab 動作（避免複雜度），但**必須**在 401 回應後妥善處理（見下）
- offcanvas 練習記錄 / history 不做 cross-tab refresh — user 切回 tab 後手動 reload 即可

**401 token expired mid-action（任何 API call 中途回 401）：**
1. 中斷當前 in-flight request，flush 進行中的 SSE
2. 把當前 view 與 form draft 寫入 `localStorage.pmDrillReturnPath`（含 phase / drill step / form 欄位 snapshot）
3. 跳出**全頁 banner**（同 mockup 15 §C B2 `banner--session`）：「登入逾時，已自動保存」+ 「重新登入」primary 按鈕
4. 點重新登入 → 跳 02 auth-flow / 登入頁；登入成功後 read `localStorage.pmDrillReturnPath` → navigate 回原 view + restore draft → 清除 `returnPath`
5. **EVAL_AUTH_ERROR / GATE_AUTH_ERROR** 等專屬 401 sub-code 不顯 error-wrap，直接走 banner + redirect 流（per SP3 spec §Error states）
6. 若 user 從 banner 點 X 關閉 → 進**唯讀模式**（form input disabled、submit-bar 隱藏 primary、保留 ghost 「上一步」與「回首頁」），再次互動才彈登入

**SSE mid-stream 中斷（Phase 2 chat 專屬）：**
- 後端送 `{done: true, turn}` 才算正常結束；無 done 而連線斷 = 異常
- UI：顯示斷線 inline tip（`.bubble--reconnect` warn-lt 米色置中）：「連線中斷，未收到完整回應。」+ 「重新發送」ghost button
- 點「重新發送」→ resend 同一 user message 觸發**新一輪 SSE**（partial 內容會被覆蓋）
- **後端 routes/circles-sessions.js 無 SSE resume 機制** — 每次 POST 都重起新串流，不追蹤 last delta；對齊 Path 2「後端不動」原則，未來如要做 resume 需獨立 sprint 加 backend stream state tracking
- 累計 3 次失敗 → 進 error-wrap `EVAL_API_ERROR`

**Phase 3 評分 loading 中切 tab/view 再回來：**
- API call 不 abort（fetch promise 持續執行）
- AppState `circlesEvaluating: true` 在 loading-wrap 顯示時設為 true，render() 任何時候都會顯示 loading-wrap（即使切到 NSM tab 再切回也是 loading 中）
- 完成時無論 user 在哪個 view，回填 AppState.circlesScoreResult；若 user 仍在 phase 3 view → render 評分頁；若已切走 → AppState 保留結果，下次切回直接顯
...
.navbar
  ...
  │  └ .navbar-tabs                    // CIRCLES | 北極星指標 (mobile @media hide)
  └ .navbar-actions
     └ guest: 登入鈕 / authed: email + 登出
```
**guest mode 與 loading mode 都必須顯示登入按鈕**（先前 loading mode 漏顯造成 bug）

**§ 2026-05-04 user override（commit `6708705`）— mobile home guest 顯示 sign-in:**
- 規格修正：mobile/tablet/desktop home 統一顯示 `<button class="navbar__icon-btn" data-nav="auth">` sign-in icon
- mockup 01 line 803 原本 mobile guest = hamburger + brand only（無 sign-in），user 親要求 override「mobile 版首頁無登入登出按鈕，這是必備規格」
- mockup 01 已同步更新（line 803-806 加 navbar__actions + sign-in button）
- `app.js renderNavbar` `isCirclesHome` 路徑改用 `signInBtn`（無 `--auth-only` modifier class）
- `style.css` 移除 `@media max-width:480px { .navbar__icon-btn--auth-only { display:none } }` rule
- **deprecated:** `signInBtnHomeOnly` 常數 + `.navbar__icon-btn--auth-only` modifier class（已無人用，保留 CSS rule 為空）
...
**§ 2026-05-04 user 親要求 — offcanvas 必須顯示進行中草稿 + 已完成兩種狀態（per mockup 09 line 304）:**
- **後端串接（user 親准呼叫既有 endpoint，routes/* 零改）:**
  - `triggerSaveCycle()` 第一次 fire 時 `POST /api/(guest-)circles-sessions/draft` body `{question_id, mode, drill_step?}` 後端 idempotent lazy-create active session 寫 `AppState.circlesSession`
  - 後續 saving phase fire-and-forget `PATCH /api/(guest-)circles-sessions/:id/progress` body `{stepDrafts, frameworkDraft}` 後端 shallow-merge `step_drafts`
  - localStorage `pmdrill:circles:draft:{qid}` 保留作 instant cache + offline fallback
- **`renderOffcanvasItem` 變體規約**:
  - `status === 'active'` (進行中草稿): meta 後綴 drill「· 草稿」/ sim「· 完整 7 步 · 進行中」/ NSM「· 4 步 · 進行中」; date 用相對時間 (< 60s 剛剛編輯 / < 60min N 分鐘前編輯 / < 24h N 小時前編輯 / < 7d N 天前編輯 / ≥7d 絕對 M/D); **不顯示 score badge**
  - `status === 'completed' || 'scored'`: 既有絕對時間 M/D + score badge
- **Score badge 文字**: 「N 分」格式（mockup 09 line 341/348/367 — 不可只顯示數字無「分」字）
- **Empty copy**: 「進行中與已完成的 CIRCLES、NSM 練習都會出現在這裡」（不可暗示只有完成才顯示）

### 2.11.1 NSM Step 1 卡片 expand 規約（mockup 06 §B + viewport-conditional runtime fix）
- **Mockup contract** (mockup 06 line 313-321): `.nsm-q-card.is-selected { border-color: navy; box-shadow: 0 0 0 1px navy, var(--shadow-2); grid-column: 1 / -1 }` — 點選後跨整行展開 `.nsm-context` 4 ctx-rows (商業模式/使用者/常見陷阱/破題切入)
- **§ 2026-05-04 desktop fix (commit `becce460` user 親要求)**: 5 卡 2-col grid + 1 expanded → `order: 999` 推末位避免 4-row 破洞，最終 2x2 + 1 row 全寬 = 3 row 整齊
- **§ 2026-05-05 mobile fix (user 親要求)**: 1-col grid 不需 order 推末位（mockup 06 §B mobile frame 規格本就是「當筆 in-place expand」），原 `order: 999` rule 在 mobile 違反 contract 造成 user 點第 1 筆看到展開卡跑到末尾
- **最終 viewport-conditional CSS**:
  ```css
  .nsm-q-card.is-selected { ... grid-column: 1 / -1; }  /* 所有 viewport 共用 */
  @media (min-width: 768px) { .nsm-q-card.is-selected { order: 999; } }  /* tablet+ only */
  ```
- 對齊 `.nsm-q-list` line 250 既有 768px 1-col→2-col grid breakpoint
...
完整 Iconography 規約 → mockup 15 §A3（每 icon 對應用途）

### 2.14 State ↔ Mockup CSS class 映射表（Bundle 0a 補洞）

每個 mockup 看起來會渲染什麼，由以下 AppState key 決定。implementer 寫 render 函式時要查此表。

**CIRCLES（mockup 03/04/05/11/12/13）：**

| AppState | 影響的 mockup class | 說明 |
|---|---|---|
| `circlesPhase` (1/1.5/2/3/4) | view 切換：phase 1 form / gate / chat / score / report | render() 路由 |
| `circlesMode` (`drill` / `simulation`) | `.mode-tag--drill` / `.mode-tag--sim`（navy 統一）/ progress bar 顯隱 | drill 隱藏 progress bar，sim 顯 |
| `circlesDrillStep` (`C1`/`I`/`R`/`C2`/`L`/`E`/`S`) | `.circles-progress__step.is-active` 對應 dot | drill 模式單步顯，sim 顯所有完成步驟 |
| `circlesStale` (boolean) | `.banner--stale`（mockup 15 §C B4）+ form 全 disabled | 來自 stale 偵測（spec §3.6） |
| `circlesLocked` (boolean) | `.banner--locked`（15 §C B3）+ submit-bar 隱藏 primary | 來自 isStepLocked() helper |
| `circlesScoreResult.dimensions[].score` | `.dim-row.is-low`（≤2 自動展開）/ `.nsm-score-row__score--low` | 客端決定 |
| `circlesScoreResult` (truthy) | mockup 11 顯實內容 / null 時顯 `.loading-wrap` | server-driven |
| `circlesEvaluating` (boolean) | mockup 11C `.loading-wrap` + `.loading-checklist`（state 機由 client 算 progress） | client-driven |
| `circlesEvaluateError` (string error code) | mockup 12 / 11D `.error-wrap` 對應 4 種 code | server-driven（EVAL_TIMEOUT / EVAL_API_ERROR / EVAL_PARSE_ERROR / REPORT_API_ERROR） |
| `circlesGateResult.overall_status` (`ok`/`warn`/`error`) | mockup 04 `.gate-transition--ok/warn/error` 三態 + 5 個 gate-item | server-driven |
| `circlesFinalReport` (object) | mockup 13 顯實 grade-card / radar / step-rows / strengths / improvements / verdict | server-driven |
| `circlesChipExpanded` (boolean) | mockup 01 q-card 展開 + mockup 03+ qchip 展開 | client-driven toggle |
| `streamingActive` (boolean) | mockup 05 `.stream-dots` + 鎖 input | server-driven SSE state |
| `circlesConversation[].role` (`user`/`coach`/`tutor`) | mockup 05 chat bubble 三種 alignment + 顏色 | server-driven |

**NSM（mockup 06/07/08/14）：**

| AppState | 影響的 class | 說明 |
|---|---|---|
| `nsmStep` (1/2/3/4) | view 路由 | render() 路由 |
| `nsmSubTab` (`nsm-step2`/`nsm-gate`/`nsm-step3`) | mockup 07/08 sub-tabs `.is-active` | client-driven |
| `nsmReportTab` (`overview`/`comparison`/`highlights`/`export`) | mockup 14 `.tab-bar__btn.is-active` | client-driven |
| `nsmGateResult.overall_status` | mockup 08 `.gate-transition--ok/warn/error` | server-driven |
| `nsmSession.scores_json.scores[k]` | mockup 14A `.nsm-score-row__score--high/mid/low` 字色 | client 算 grade |
| `nsmActiveCompareNode` (string key) | mockup 14B' `.nsm-compare-card.is-active` + `.nsm-coach-detail` 顯示 | client-driven |
| `nsmContext` (object) | mockup 06 q-card `.context-card` 4 欄；null 顯 loading | server-driven (SP4 預生成) |

**Global / 跨頁：**

| AppState | 影響 | 說明 |
|---|---|---|
| `view` | 整個 #app 內容（'circles' / 'nsm' / 'auth' / 'history' deprecated） | router |
| `accessToken` (truthy) | navbar 右上「登入」消失 / 出現 email | auth state |
| `guestId` | guest header 用；若有 token 就無視 | client only |
| `isOnline` (boolean) | mockup 15 §C B1 offline banner 顯隱 | window event listener |
| `sessionExpired` (boolean) | 15 §C B2 banner | 401 trigger |
| `onboardingComplete` (boolean) | mockup 10 是否顯示 | localStorage |

**Button → AppState 變化全集（mockup 15 §A3 補充）：**

| Button | onClick action |
|---|---|
| 「再練一題」(11/13/14 submit-bar) | reset CIRCLES/NSM state + 隨機抽 1 題 + auto-select + navigate home |
| 「重試」(error-wrap) | clear error + 重發同 API（不換 question） |
| 「上一步」(03/04/07/08 ghost) | `circlesPhase--` 或 sub-tab back（per render context） |
| 「返回修改」(04/11/12/13 gate/error) | `circlesPhase = 1` 直接回 form（保留 draft） |
| 「回首頁」(navbar / submit-bar) | reset session state + navigate('circles') |
| 「隨機抽 5 題」(01/06 reshuffle) | `pickRandom5(QUESTIONS)`，**不**切 view，**不**清除選中卡 |
| 「匯出 PNG」(13) | 用 html2canvas 截 `.circles-final-report` → 下載；完成顯 toast |
| 「點擊看思路」(14B coach card) | set `nsmActiveCompareNode`，render 顯 detail panel；mobile 開 bottom-sheet |
...
9. **`renderHistory()` + `renderHistoryChart()` + `renderHistoryList()` 全頁 history view（app.js:5689 / 5819 / 5903）→ DEPRECATED**（Bundle 0a 補洞）
   - 16 mockup 完全用 offcanvas（mockup 09）取代全頁 history
   - **重寫時動作**：刪除 `case 'history'` view router（app.js:856 附近）+ 三個 render 函式 + 相關 binders
   - 14-day 練習曲線（renderHistoryChart）一併砍 — Path 2 不再有此功能（後續若 user 要可作為新 sprint）
   - localStorage `lastView === 'history'` 開機自動 redirect 到 'circles'
...
| 01 | ✅ **放行 v5** | `docs/superpowers/specs/mockups/2026-05-02-frontend-rewrite/01-circles-home.html` | CIRCLES Home — A 預設 / B drill mode / C 搜尋有 / D 搜尋無 / E loading / F edge cases / **G 題目展開（card-based blocks — §0.7.1 pattern）**。**v5 對齊缺口補**（§ 2026-05-03）：（1）補 **NSM cross-promo card** 在 home 底部（mobile 直式 stacked / tablet+desktop 橫式 with media query）；（2）補「什麼是 CIRCLES 實戰訓練？」**accordion**（精簡 2-line prose：CIRCLES 七步框架釐清→鎖定→列舉→排序→提出→取捨→總結 + 完整模擬/步驟加練模式對照）；（3）**wording 全對齊 production**：個別步驟 → 步驟加練、設計/改善/策略 → 產品設計 ×40 / 產品改進 ×35 / 產品策略 ×25；（4）**q-card title 全改公司·產品/feature 格式** —「Spotify · Spotify Podcast」/「Notion · 工作協作」/「Airbnb · Marketplace 擴展」（25 處），drill 模式保留「— 練 C 步驟」suffix；（5）**mode-tag 統一 navy**（去除 `mode-tag--sim` 的 success 綠，與 mode-tag--drill 一致）。**mobile navbar 標準化（橫跨 04-10）**：navy filled 28px 圓角 box + ph-circles-three white icon + 「PM Drill」text，84 處全替換 |
...
| 10 | ✅ 放行 | `docs/superpowers/specs/mockups/2026-05-02-frontend-rewrite/10-onboarding.html` | Onboarding 引導 — **5 sections × 3 viewports = 15 frames**。**A 歡迎卡**：navy hand-waving icon + 「歡迎來到 PM Drill」+ 介紹文 + 兩 button「開始引導 → primary」/「直接自己選題 ghost」。**B 引導 step 1 練習模式** / **C 引導 step 2 選擇題型** / **D 引導 step 3 挑一道題目** / **E 引導 step 4 開始練習**。**Spotlight 機制**：`.onb-targeted` class 直接掛在目標元素，box-shadow 同時做 ring + outer dim，auto-sized 永遠對齊，無雙重 dim。**Dual ring 配色**：2px 白 inner halo + 6px navy outer ring（mobile 1px+4px）+ 9999px outer rgba(20,15,10,0.45) dim — 對任何 target 顏色都清楚（解決 navy button 撞色問題）。**Mobile pattern override** § 2026-05-03 user 定案：mobile 採 desktop 同 pattern（全頁 dim + 浮動 tooltip near target with arrow），不再 sticky-bottom。**Step 4 UX 修正**：spotlight 圍整張 expanded q-card（含 公司+描述+button），tooltip 文案「先讀題目說明再決定：合適就點『確認，開始練習』進入 Phase 1，不合適可上一步換題」 — 避免強迫使用者直接點按鈕。Tour step labels 全照 production `ONBOARDING_STEPS` 規格。Component CSS LOCKED：root tokens / navbar / btn 全段 copy 自 03 |
| 11 | ✅ 放行 | `docs/superpowers/specs/mockups/2026-05-02-frontend-rewrite/11-phase-3-score.html` | Phase 3 步驟分數 — **4 sections × 3 viewports = 12 frames**。**A 預設**（drill mode I 步驟完成）：navbar + circles-nav（back + 「I 步驟評分結果」+ Spotify · Spotify Podcast）+ progress-bar（C done · I active · R-S pending）+ score-total 大數字「78」（Instrument Serif italic 80px navy）+ 副文「I — 用戶分析 步驟得分」+ 4 dim breakdown collapsible（清晰度 4/5 / 邏輯性 3/5 / 完整度 3/5 / 洞察力 4/5；每 dim：name + score X/5 + bar；mobile/tablet 預設折疊 / desktop 全展開）+ highlight grid（最強表現 success 綠 / 最需改進 warn 橘）+ 教練示範 toggle（折疊）+ submit-bar（回首頁 ghost / 再練一題 primary with ph-shuffle，無 sticky 黃色「恭喜完成」card per § C）。**B 低分自動展開 + 教練示範全展開**：邏輯性 1/5 `is-low` warn 橘 bar + score + auto `is-open`（comment + coach-version + tip）/ coach-demo `is-open` 顯示 3-section coachVersion object 結構（① 為什麼這個步驟重要 = 情境前置 / ② 逐欄位示範 = 4 coach-field 卡 / ③ 為什麼這樣答 = quote-style reasoning）。**C Loading**：56px navy spinner + 「正在生成評分」+ 副文 + 4-step checklist（解析框架 done / 計算分數 active / 生成示範答案 pending / 整理建議 pending）/ **D Error**：64px 紅圓 ph-cloud-warning + 「評分生成失敗」+ 副文「AI 服務暫時無法回應，你的答案已自動保存」+ error-code「EVAL_TIMEOUT」+ 重新評分 primary / 返回修改答案 ghost。**Desktop 2-col layout**：score-col-left（380px flex column 含 score+highlights+coach）+ dim-list（flex 1）— 用 flex + display:contents + CSS order 規則保證 mobile/tablet 維持「score → dim → highlights → coach」DOM 順序，desktop 透過 flex-direction:row 變成 2-col；避免 grid-row span 撐高 col-1 留下大量空白的問題。circles-nav 不放重複 home button（navbar 右上 home 唯一） |
| 12 | ✅ 放行 | `docs/superpowers/specs/mockups/2026-05-02-frontend-rewrite/12-phase-3-error-loading.html` | Phase 3 Error 變體 + Loading 慢回應 — 補 11 未涵蓋的 error codes 與 slow state（11 已有 Loading 基本 + EVAL_TIMEOUT；production 目前用 plain `alert()`，無專屬 UI）。**3 sections × 3 viewports = 9 frames**。**A Loading 慢回應**：spinner + checklist 推進至 step 3「生成示範答案」active（前兩步 done）+ `loading-sub--slow` 變體（warn 橘色 inline 文字，**不用 toast 框**）「比預期慢一些…AI 深度分析中，偶而會需要比較久時間，請再等等。」**觸發時機 60s 後切 warn 變體；實際 timeout 300s（5 分鐘，內部設定不告知使用者）才進 error 頁**（per § 2026-05-03 user 定案，避免顯示具體秒數給 user 太嚴格的時間壓力）。**B Error `EVAL_API_ERROR`**：紅圓 cloud-warning + title「評分服務暫時不可用」+ 副文「我們的伺服器忙線中，請稍候片刻。你的答案已自動保存。」+ error-code + 重新評分/返回修改答案。**C Error `EVAL_PARSE_ERROR`**：title「教練回應格式異常」+ 副文「AI 回傳的內容無法正確解析。重試通常能解決，或返回修改答案。」+ error-code + 同 actions。**EVAL_AUTH_ERROR (401) 直接跳登入頁不顯錯誤頁**（per SP3 § Error states）。Component CSS LOCKED copy 自 11（loading-wrap / error-wrap / circles-nav / progress / btn / navbar）；新增 `loading-sub--slow` inline warn 變體 |
| 13 | ✅ 放行 | `docs/superpowers/specs/mockups/2026-05-02-frontend-rewrite/13-phase-4-final.html` | Phase 4 模擬面試總結報告 — **3 sections × 3 viewports = 9 frames**。**A 預設 77 分**：navbar + circles-nav「模擬面試總結報告 · Spotify · Spotify Podcast」+ qchip + grade-card（**無 grade letter A/B/C/D，僅大字 Italic 77 navy 88px + 分 + headline 兩行帶具體判讀**：「七步框架掌握扎實，整體論述清晰；最強在 I 用戶與 L 方案，下一步建議補強 E 取捨的量化能力」）+ top-grid（mobile/tablet 直式 / desktop 2-col：左 radar 右 step-rows）+ 7-axis radar **navy polygon（去紫，rgba(27,45,92,0.16) fill + navy stroke）** + step-rows 7 行**每步補一行 commentary**（C 78 時間框架 / I 82 最強亮點 / R 75 補競品對比 / C2 70 缺 RICE/ICE / L 85 紮實 / E 68 本次最弱量化不足 / S 80 tracking 完整）+ **S 總結 row 內嵌 NSM 4 dim mini cards**（dashed border 視覺從屬 S 步驟，明確表達 4 dim 是 S 步驟拆解內容、非獨立 top-level section；4 dim 全部統一中性樣式 bg-soft + Italic 1/2/3/4 編號區分，**不用顏色區分**避免雜亂；mobile/tablet/desktop 一律 1-col 直式）+ 表現優秀（success 綠 + ph-check-circle）+ 需要改進（warn 橘 + ph-warning-circle）+ 教練總評（navy + ph-graduation-cap）+ nextsteps row + submit-bar sticky（**「再練一題」**換題 primary、不重做同題；匯出 PNG ghost / home icon-only）。**B Loading**：56px navy spinner + 4-step checklist（彙整七步驟資料 done / 計算總分與評等 active / 生成 7-axis 雷達圖 pending / 整理改進建議 pending）/ 無 submit-bar。**C Error**：64px 紅圓 ph-cloud-warning + 「報告生成失敗」+ 副文「總結報告 API 暫時不可用，你的七步驟評分已自動保存」+ error-code「REPORT_API_ERROR」+ 重試 primary / 回首頁 ghost。Component CSS 整段 LOCKED copy 自 03 / 04 / 11 / 12（navbar / circles-nav / qchip / submit-bar / btn / loading-* / error-*）|
| 14 | ✅ 放行 | `docs/superpowers/specs/mockups/2026-05-02-frontend-rewrite/14-nsm-step-4.html` | NSM Step 4 報告 — 4 tabs（總覽 / 對比 / 亮點 / 完成）+ 教練思路展開 = **5 sections × 3 viewports = 15 frames**。共用 chrome：navbar + nsm-nav「NSM 報告 · Spotify · Spotify Podcast」+ nsm-summary（Italic 大字 80 navy 56px + / 100 + 公司名）+ tab-bar 4 tabs underline-style。**A · Tab 1 總覽**：5-axis pentagon radar **navy polygon（去紫，rgba(27,45,92,0.16) fill + navy stroke）** + 5 score rows（價值關聯 4/5 / 領先指標 4/5 / 操作性 5/5 / 可理解性 4/5 / 週期敏感 3/5；每 row：name + serif Italic 22px score + max + navy bar + 教練 comment 一行；score 字色 grade-based ≥4 success / 3 navy / ≤2 warn；**bar 統一 navy uniform** 去原 5 色雜亂）；mobile/tablet 直式 / **desktop 2-col**（380px radar 卡 / 1fr score-rows 卡）。**B · Tab 2 對比預設**：mobile 5 dim block（dim title + 你的 grey 邊 + 教練 navy 邊上下堆疊）；tablet+desktop 2-col grid（header row「你的拆解 / 教練版本（點擊看思路）」+ 5 列 = NSM + 觸及廣度 / 互動深度 / 習慣頻率 / 留存驅力，每列 140px label-key（serif italic）+ 你的 + 教練）。**B' · Tab 2 教練思路展開**：點擊教練 card → `.is-active` highlight（navy 邊光 + navy-lt bg）並開啟思路 panel；mobile **bottom-sheet drawer**（handle bar + close + 兩段內容）；tablet+desktop **inline panel** 插在被點 row 下方（白底 + navy left border + nsm-coach-detail__icon 圓 graduation-cap + 教練思路 + 為什麼這樣拆解 quote）。**配色 navy 中性，去 production 原黃色 #FFF8E7**（user 不喜歡）。**C · Tab 3 亮點**：4 cards — 最大亮點（success 綠 + ph-trophy）/ 主要陷阱（warn 橘 + ph-warning-circle）/ <strong>下一步建議</strong>（NEW navy + ph-arrow-right + border-left success）/ 總評（neutral grey + ph-chat-text）；mobile 1-col 4 cards 直堆 / tablet 2-col 前 2 並排 + 下一步建議跨 2 + 總評跨 2 / desktop 3-col 並排 + 總評跨 3 欄。**D · Tab 4 完成**：取代 production「只有再練一次按鈕 + 空白」barebones — done-panel（ph-check-circle 64×64 success-lt 圓 + 「完成這次 NSM 訓練」+ 「本次得分 80 分，距離滿分還差 20 分；表現扎實 4/5 個維度達標」+ 主按鈕「再練一題 ph-shuffle」+ desktop 加 ghost「回首頁」）+ done-secondary info card（「NSM 練習小技巧」+ 3 條 bullets）。**用顯式 modifier class 控制 layout**（`.nsm-overview--desktop` / `.nsm-compare--stack` vs `--grid` / `.nsm-highlights--tablet` vs `--desktop`），**不用 @media** — 原因：mockup 三 viewport frame 並排於 1920px 真實 viewport，@media 規則所有 frame 都觸發 desktop 樣式導致 bug。Component CSS 整段 LOCKED copy 自 03/04/06/07/11/13（navbar / nsm-nav / btn / panel-card）|
| 15 | ✅ 放行 | `docs/superpowers/specs/mockups/2026-05-02-frontend-rewrite/15-error-empty-collation.html` | Error / Empty / Loading 全集對齊 + **cross-mockup audit 22 條 drift 收尾**（13 HIGH / 6 MED / 3 LOW）。**§A 規約字典 3 表 凍結唯一寫法**：Copy 對照（再練一題 / 重試 / 上一步 / 返回修改 / 回首頁 / 隨機抽 5 題 / NSM 報告 / Loading 標題模板 `{動詞}{對象}中…`）+ Loading Timeout（`0-60s` 正常 / `60s+` slow `loading-sub--slow` warn 橘 inline / `300s` 硬上限不告知 user / NSM gate 例外典型 8-15 秒 / Phase 4 典型 30-60 秒）+ Iconography（ph-shuffle 換新題 / ph-arrow-clockwise 重試 / ph-arrow-left 上一步 / ph-arrow-right 下一步 icon 永遠在文字後 / ph-house 回首頁 / ph-cloud-warning API error / ph-warning-circle 改進 / ph-check-circle success / ph-info / ph-wifi-slash offline / ph-circles-three navbar）。**§B Loading 全集** 3 viewports：L1 page loading-wrap（同 11/12/13）/ L2 inline btn loading（btn 內 14px spinner，btn disabled，**audit punch 19 補洞**）/ L3 streaming dots（chat typing pulse animation）/ L4 skeleton card（shimmer 灰塊 1/2/3-col）。**§C Error & Banner 全集** 3 viewports：E1 page error-wrap 全 4 種 error code（EVAL_TIMEOUT / EVAL_API_ERROR / EVAL_PARSE_ERROR / REPORT_API_ERROR）+ B1 network offline banner bg-deep（**audit punch 18 補洞**）+ B2 session expired warn-lt 米色（**audit punch 17 補洞**）+ B3 locked banner navy-lt（已得分鎖定）+ B4 stale banner danger-lt（題庫更新必擋）+ form-field 錯誤 inline border 紅 + tip。**§D Empty / Toast / Modal** 3 viewports：M1 list empty（offcanvas 無記錄）/ M2 search 無結果 / M3 filter 無符合（**audit punch 20 NSM step 1 empty 補洞**）+ toast 4 變體 navy/success/warn/danger left-border 白底（**audit punch 16 補洞 — 全 14 mockup 第一次出現 toast**）+ confirm modal danger primary（**audit punch 22 補洞**，刪除確認）。**同 commit patch 5 既有檔**：00（deprecate `--c-accent-nsm` token + 4 swatch + badge/mode-tag/CIRCLES vs sim 註解 + EVAL_TIMEOUT 30s 文案改 300s 規約）/ 01（拔紫 token line）/ 03（4-dim 註解 #3b82f6 / #8b5cf6 hex → navy + 1/2/3/4 編號）/ 06（attention purple → navy）/ 02 auth-submit 留 future work 註記（規模大不在此 commit 處理）。**§A 後續擴張 6 表**（Bundle 0c 補洞）：A4 Class Naming 規約（已存在 mockup HTML 的 class 全 LOCKED；deprecate `.btn--home` `.auth-submit`）+ A5 Button → AppState 跳轉（10 條按鈕對照）+ A6 Copy 常數 extract 規則（≥3 函式重複才 extract）|
| 16 | ✅ 放行 | `docs/superpowers/specs/mockups/2026-05-02-frontend-rewrite/16-flow-transitions-edge.html` | Flow transitions + edge cases — **Bundle 0b 補洞**，readiness review 揭示的 4 條 cross-screen transition / edge state，**4 sections × 3 viewports = 12 frames**。**§A Drill「再練一題」跳轉**：Phase 3 score 按 primary → reset CIRCLES session + `pickRandom5` 5 題 + auto-select 1 + navigate('circles') home with new q-card 展開（**不**清 onboarding flag、**不**清歷史）。**§B Simulation 步驟間 transition**：Phase 3 score 按「下一步 R 需求 →」→ `circlesPhase = 1` + `circlesSimStep++` → progress bar dot N done / N+1 active / N+1 之後 plain；qchip 持續顯題；done step 可點 dot 跳該步 phase 1 form read-only + locked banner（per spec §2.4）。**§C Phase 2 SSE 中斷 + 重新發送**：streaming 中網路斷（無 `{done: true}`）→ chat body 末尾顯 `.bubble--reconnect`（warn-lt 米色置中）「連線中斷，未收到完整回應」+「**重新發送**」ghost；點下後 resend user message **重啟新一輪 SSE**（後端 routes/circles-sessions.js 無 resume 機制，partial 內容會被覆蓋）；累計 3 次失敗進 EVAL_API_ERROR；input 鎖定。**§D Phase 3 loading 中切到別頁回來**：in-flight fetch 不 abort，`circlesEvaluating: true` 持續，render() 任何時候都顯 loading-wrap；切到 NSM tab 顯 resume-toast「CIRCLES 評分仍在背景進行中」；切回 phase 3 view 顯 (A) 還沒回 → loading / (B) 已完成 → 評分頁 / (C) 失敗 → error-wrap 三種 render 結果（per §1.5.1）。**原 §E Migration flow 已刪**：production `/api/migrate-guest` 是 silent background fetch（只搬 lastSessionId 1 筆，errors swallowed，無 progress / 無 toast），對齊「後端不動」鐵則無 UI 可 mockup |
