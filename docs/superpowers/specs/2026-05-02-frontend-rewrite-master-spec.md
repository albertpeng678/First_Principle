...
- 後端送 `{type:"end"}` 才算正常結束；無 `{type:"end"}` 而連線斷 = 異常
- UI：顯示斷線 inline tip（同 chat bubble，但 grey + 「連線中斷，已收 X 字。」+ 「繼續」ghost button）
- 點「繼續」→ resend 同一 user message 觸發新 SSE（後端會看到 already-streaming session 並從 last delta 接續，per app.js:Phase 2 implementation）
...
| 16 | ✅ 放行 | `docs/superpowers/specs/mockups/2026-05-02-frontend-rewrite/16-flow-transitions-edge.html` | Flow transitions + edge cases — **Bundle 0b 補洞**，readiness review 揭示的 5 條 cross-screen transition / edge state，**5 sections × 3 viewports = 15 frames**。**§A Drill「再練一題」跳轉**：Phase 3 score 按 primary → reset CIRCLES session + `pickRandom5` 5 題 + auto-select 1 + navigate('circles') home with new q-card 展開（**不**清 onboarding flag、**不**清歷史）。**§B Simulation 步驟間 transition**：Phase 3 score 按「下一步 R 需求 →」→ `circlesPhase = 1` + `circlesSimStep++` → progress bar dot N done / N+1 active / N+1 之後 plain；qchip 持續顯題；done step 可點 dot 跳該步 phase 1 form read-only + locked banner（per spec §2.4）。**§C Phase 2 SSE 中斷 + reconnect**：streaming 中網路斷（無 `{type:"end"}`）→ chat body 末尾顯 `.bubble--reconnect`（warn-lt 米色置中）「連線中斷，已收 X 字」+「繼續」ghost 重發 user message → 後端從 last delta 接續（per §1.5.1）；累計 3 次失敗進 EVAL_API_ERROR；input 鎖定。**§D Phase 3 loading 中切到別頁回來**：in-flight fetch 不 abort，`circlesEvaluating: true` 持續，render() 任何時候都顯 loading-wrap；切到 NSM tab 顯 resume-toast「CIRCLES 評分仍在背景進行中」；切回 phase 3 view 顯 (A) 還沒回 → loading / (B) 已完成 → 評分頁 / (C) 失敗 → error-wrap 三種 render 結果（per §1.5.1）。**§E Migration flow**：guest 註冊後讀 localStorage `guestId` POST `/api/migrate-guest`（per §1.1）→ migration-wrap 顯 (A) 「把 8 筆練習記錄搬到帳號？」確認 + 搬移 primary（desktop 加 ghost「跳過」）→ (B) 進度 bar 5/8 + checklist（讀取 → 對應 user_id → 更新 DB → 刪 localStorage guestId）→ (C) 完成 success state + 「進入首頁」 / 失敗 retry（MIGRATE_API_ERROR）|
