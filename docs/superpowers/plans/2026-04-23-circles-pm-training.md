...
## Task 19: Playwright Audit

**Files:**
- Modify: `tests/` (existing Playwright test files)

- [ ] **Step 1: Add CIRCLES tests to existing Playwright spec**

Find the existing Playwright test file (likely `tests/` directory). Add a new spec file `tests/circles.spec.js`:
...
const BREAKPOINTS = [
  ...
  { name: '430-pro',     width: 430,  height: 932 },
...
for (const bp of BREAKPOINTS) {
  test.describe(`CIRCLES @ ${bp.name}`, () => {
    test.use({ viewport: { width: bp.width, height: bp.height } });

    test('home shows CIRCLES tab', async ({ page }) => {
      await page.goto(BASE);
      await page.waitForSelector('.home-tab-btn');
      // Find the circles tab
      const circlesTab = page.locator('.home-tab-btn', { hasText: 'CIRCLES' });
      await expect(circlesTab).toBeVisible();
    });

    test('CIRCLES tab navigates to question picker', async ({ page }) => {
      await page.goto(BASE);
      await page.waitForSelector('.home-tab-btn');
      await page.locator('.home-tab-btn', { hasText: 'CIRCLES' }).click();
      const startBtn = page.locator('#btn-circles-start');
      await expect(startBtn).toBeVisible();
      await startBtn.click();
      await expect(page.locator('.circles-home-title')).toBeVisible();
    });

    test('mode selector switches between drill and simulation', async ({ page }) => {
      await page.goto(BASE);
      await page.locator('.home-tab-btn', { hasText: 'CIRCLES' }).click();
      await page.locator('#btn-circles-start').click();
      await expect(page.locator('.circles-mode-card[data-mode="drill"]')).toBeVisible();
    ...
    test('type tabs filter questions', async ({ page }) => {
      await page.goto(BASE);
      await page.locator('.home-tab-btn', { hasText: 'CIRCLES' }).click();
      await page.locator('#btn-circles-start').click();
      await page.locator('.circles-type-tab[data-type="improve"]').click();
      await expect(page.locator('.circles-type-tab[data-type="improve"].active')).toBeVisible();
    });

    test('question selection opens Phase 1 form', async ({ page }) => {
      await page.goto(BASE);
      await page.locator('.home-tab-btn', { hasText: 'CIRCLES' }).click();
      await page.locator('#btn-circles-start').click();
      const firstQ = page.locator('.circles-q-card').first();
      await firstQ.click();
      await expect(page.locator('.circles-field-input').first()).toBeVisible();
    });

    test('Phase 1 submit button is visible and has min touch size', async ({ page }) => {
      await page.goto(BASE);
      await page.locator('.home-tab-btn', { hasText: 'CIRCLES' }).click();
      await page.locator('#btn-circles-start').click();
    ...
    test('NSM tab still works after CIRCLES integration', async ({ page }) => {
      await page.goto(BASE);
      await page.waitForSelector('.home-tab-btn');
      await page.locator('.home-tab-btn', { hasText: '北極星' }).click();
      const nsmPanel = page.locator('.nsm-tab-panel').first();
      await expect(nsmPanel).toBeVisible();
    });

    test('resume card appears for in-progress session', async ({ page, context }) => {
      // Start a session
      await page.goto(BASE);
      await page.locator('.home-tab-btn', { hasText: 'CIRCLES' }).click();
      await page.locator('#btn-circles-start').click();
      await page.locator('.circles-q-card').first().click();
      // Fill all 4 Phase 1 fields
      const inputs = page.locator('.circles-field-input');
      const count = await inputs.count();
      for (let i = 0; i < count; i++) {
        await inputs.nth(i).fill('測試填入內容 ' + i);
      }
      // Do NOT submit — just navigate away to simulate abandon
      await page.goto(BASE);
      // Reload and go back to CIRCLES
      await page.locator('.home-tab-btn', { hasText: 'CIRCLES' }).click();
      await page.locator('#btn-circles-start').click();
      // Resume card might need a moment to load
      await page.waitForTimeout(1500);
      // Either resume card or question list should be visible
      const resumeCard = page.locator('.circles-resume-card').first();
      const qList = page.locator('.circles-q-list');
      await expect(qList).toBeVisible(); // at minimum, question list renders
    });
  });
}
```
...
```bash
npx playwright test tests/circles.spec.js --reporter=list
```
Expected: All tests pass at all 5 breakpoints.

- [ ] **Step 3: Fix any failures**

For each failing test:
- Read the error message
- Identify the root cause in `app.js` or `style.css`
- Fix it
- Re-run until green
...
```bash
git add tests/circles.spec.js
git commit -m "test(circles): add Playwright audit specs for all 5 breakpoints"
```
