...
| Task | 性質 | 為什麼 | 工時 |
|---|---|---|---|
| **C-T1 find** | AI 服務壞掉的錯誤狀態盤點（OpenAI 500 / timeout / rate-limit × NSM gate / NSM evaluator / CIRCLES gate / CIRCLES evaluator 4 處）| 直攻 99.9% conversion 流失推測根因 | 3h |
| **C-T2 find** | 99.9% NSM conversion 線上資料深挖（query Supabase service-role + 寫 1 spec 走完整 NSM 記錄每步 timing + drop signal）| 看到實際在哪步斷掉 | 4h |
| **B10 / O-6 fix** | `_doOffcanvasDelete` invalidate `circlesRecentSessions`（3 行 fix）| 已知 root cause 可直接 fix | 30min |
| **B6 / #21 fix** | mockup 04 audit + 9 transition drift 修 | 視覺契約 drift 累積會破 17 mockup 規矩 | 2-4h |
| **B13 / O-11 find+fix** | adversarial 擴 3 prompt（`circles-conclusion-check` / `circles-final-report` / `circles-coach-version`） | 4-pillar → 7-pillar 完整 | 2-4h |

**Phase 1 出口條件**: tracker §1 多 N 條 AI-error + conversion finding，user 看過 + 決定 Phase 1B 修哪些。
