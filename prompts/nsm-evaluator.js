...
async function evaluateNSM({ question_json, user_nsm, user_breakdown }) {
  ...
  const productTypeGuide = {
    attention:   '注意力型（社交/媒體/遊戲）：廣度=觸碰核心功能的 MAU，深度=時長/完播，頻率=DAU/MAU 比',
    transaction: '交易量型（電商/共享平台）：廣度=活躍供給方，深度=需求方每月交易次數，頻率=成交轉化率',
    creator:     '創造力型（UGC/知識平台）：廣度=活躍創作者數，深度=成果品質/互動數，頻率=內容被廣泛閱讀比例',
    saas:        'SaaS 型（B2B/訂閱）：廣度=啟用（Activation）率，深度=席次利用率，頻率=工作流黏著率',
  };
...
凡 user_nsm 或任一 user_breakdown 維度（reach/depth/frequency）符合以下任一條件：
...
若 4 個欄位**全部**觸發 → 5 維度 alignment/leading/actionability/simplicity/sensitivity 全 score=1，totalScore = 20。
