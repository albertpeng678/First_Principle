...
test.describe('B1 CIRCLES Home', () => {
  ...
  // user 2026-05-04 親要求 override mockup 01 line 803：
  // mobile home guest 應顯示 sign-in icon（所有 viewport 一致）
  // renderNavbar 改用 signInBtn（無 --auth-only class），sign-in 全 viewport 可見。
  test('mobile-360 CIRCLES home guest navbar shows sign-in (user 2026-05-04 override)', async ({ page }) => {
    await page.setViewportSize({ width: 360, height: 800 });
    await page.goto('/');
    await page.waitForSelector('.qcard');
    const signInBtn = page.locator('.navbar__icon-btn[data-nav="auth"]');
    await expect(signInBtn).toBeVisible();
  });

  test('tablet-768 CIRCLES home guest navbar shows sign-in (all viewports)', async ({ page }) => {
    await page.setViewportSize({ width: 768, height: 1024 });
    await page.goto('/');
    await page.waitForSelector('.qcard');
    await expect(page.locator('.navbar__icon-btn[data-nav="auth"]')).toBeVisible();
  });
});
