...
> **Last updated:** 2026-05-08（**Post-ship 96-PNG audit ✅ DONE + 8-vp regression complete 2112/2160 + 2 real regressions FIXED** — 96 PNG opus 親 Read 8 vp：mockup 04 32/32 + 07 32/32 + 10 40/40，3 mockups SHIP-READY；4 drifts 🟡 follow-up（mockup 04 9 transition copy/icon × Task 21 / DRIFT-07-1 nsm-context-card__hint missing / DRIFT-10-1+10-2 step3 selector + step4 expand）；iOS 15-item × 3 全 PASS；Combo C adversarial defense earlier 50/50 GREEN；**regression 8-vp 完跑 23.7 min — 2 real regressions found+fixed**：(1) `circles-home.spec.js:8-19` add `/api/guest-circles-stats` stub mirror（Bug A 後 fixture stale）（2) `public/app.js:965-977` Combo C minLength 與 locked/stale/saveError replace 順序衝突 — 改 minLength gate 跑在最後（保留 23/23 phase1-locked-stale + min-length-frontend 全綠 Desktop-1280）；32 其他 fail = nsm-home race flake + nsm-card-inplace-expand:87/101 desktop-viewport pre-existing fragile（隔離 4/4 pass / 2/6 fail，產品 CSS 對，spec locator 設計需收斂）；audit：`audit/eyeball-mockup-{04,07,10}.md` + `audit/session-report-2026-05-08-post-ship-audit.md`）
>
