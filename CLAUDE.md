...
> **Last updated:** 2026-05-08（**Combo C — Adversarial Input Quality + Frontend minLength ✅ SHIPPED** — User 親抓 P0 bug「Phase 1 form 4 欄全打 A → AI 回 1 ok hallucination」根因調查：Playwright capture spec 用 mockApis() 沒驗真 AI；user 親准 Path 2 carve-out 動 5 prompts；3-Layer 守門 — L1 frontend minLength + L2 5 prompts 加 ## 輸入品質檢查 + L3 real-OpenAI adversarial sweep 50/50 GREEN；jest 143/143 baseline 不破；Playwright +13 new specs；total OpenAI cost 開發期 ~\$1.25-1.50；commits `1c97657 → 56ddd88 → a68deb4 → bf172f0 → 5e85c82 → b41b3d6 → deae14c → cce56f0 → 6135e09 → 6d217fe`；standing rule 入 memory：所有 AI 審核階段 ship 前必跑 adversarial sweep）
>
