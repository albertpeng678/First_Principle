...
> **Last updated:** 2026-05-08（**Plan B Phase 3 score (mockup 11) ✅ MERGED** — `bf42fda..b819b2c` 4 commits：P3-1 AppState 4 fields + Phase 3 router / P3-2+P3-3 Section A+B score-card + dim-rows + highlights + multi-section coach-demo (auto-expand dim ≤ 2 + auto-open coach-demo for low-score) / P3-4 Section C Loading 56px spinner + 4-step checklist setInterval 5s + 30s timeout fallback + Section D Error cloud-warning + error-code-badge + retry wire / P3-5 12 production PNGs + iOS 15-item + audit doc。**全裝置 8-vp 親跑驗證**：phase3-score **272/272 pass × 8 viewports**（34 specs × 8 = 42.7 s）+ jest 143/143 baseline 不破 + critical regression 86/86 Desktop-1280。**Opus director cold-Read 4 production PNGs**：A-mobile (78 + 4 dim collapsed) / B-desktop (52 + 邏輯性 1/5 auto-expand + coach-demo 全展開) / C-tablet (spinner + 4-step) / D-desktop (cloud-warning + EVAL_TIMEOUT badge) — 全對齊 mockup baseline 0 drift。**3 🟡 non-blocking concerns**：A-mobile coach-demo 需 scroll、`_phase3CoachDemoInitialized` side-effect code smell、window-scope timer（已用 clearPhase3Timers 兜底）。**Pre-implementation 12-PNG audit**：opus 親 Read mockup 11 全 4 sections × 3 vp 建立視覺契約 → spec + plan in commit `574a764`。**Earlier 同日**：Plan B Phase 2 全 6 sections (mockup 05) 7 commits MERGED + post-ship 96-PNG audit + 2 regressions fixed (Combo C)。docs：`audit/eyeball-mockup-11.md` + `audit/png-prod-mockup-11/` + spec/plan in `docs/superpowers/specs+plans/2026-05-08-*-phase3-*.md`）
>
...
| Post-ship hotfix（user 親要求 — Stats 0/0/0 guest + Phase 1 restore 全 7 步全面修復）| ✅ DONE — 8 commits `e01dcc0..6411606`。**Bug A** root cause：`/api/circles-stats` 是 `requireAuth` only，guest 用戶 401 → frontend 早退 → home stats-strip 永遠 `0 已完成 · 0 進行中 · 0 本週`。**修復（user 親准 surgical 後端 add-on，比照 `2607720` 先例）**：(1) 新 `routes/guest-circles-stats.js` 鏡像 auth route，用 `requireGuestId` middleware；(2) `server.js:43` 加 mount line（不改 auth route）；(3) `public/app.js:2109` 1-line 改 `var path = AppState.accessToken ? '/api/circles-stats' : '/api/guest-circles-stats'`。**Bug B** root cause：`populateTextareasFromDraft` C/I/R/C2 block (`public/app.js:2552-2573`) 有 `|| draftValues[fieldIdx]` positional `Object.values()` fallback — partial-fill / 鍵插入順序不同 / 空字串 / 舊英文 key 都會 trigger fallback → 值跑錯欄位。**修復**：(1) 移除 positional fallback；(2) 加 16-key `ENGLISH_ALIAS` 兼容舊 session（`boundaryScope/timeWindow/businessImpact/assumption/...` 共 C1+I+R+C2 16 key）；(3) L/E/S 步 audit 確認 push/pop dense + 雙鍵 + 純 key map 結構安全 — 無 code 改動只加 4 條 regression spec。**全 7 步覆蓋 11 條 partial-fill spec**（C1 × 3 / I/R/C2 × 3 / 英文 alias × 1 / L/E/S × 4）。後端不動鐵則 Bug B 完全遵守（純前端）。jest 160/160（+3 from new guest stats route）/ Playwright `restore-no-drift` 11 × 8 vp = 88 全綠 / `home-stats-guest` 4 × 8 vp = 32 全綠 / `offcanvas-item-click-restore` 15/15 regression baseline 全綠（English-key fixtures 經 alias path 仍綠）/ 6 PNG opus 親 Read（stats × 3 vp + restore-c1 × 3 vp，confirm「測試」精準停在時間範圍 idx 1，業務影響 idx 2 placeholder visible 無 drift）/ iOS 15-item 15/15 PASS（純資料修，無 UX surface 改動）/ ENGLISH_ALIAS 16/16 key cross-check 對齊 `CIRCLES_STEP_CONFIG.{C1,I,R,C2}.fields[].key`。Spec：`docs/superpowers/specs/2026-05-07-stats-and-restore-fix-design.md` / Plan：`docs/superpowers/plans/2026-05-07-stats-and-restore-fix.md` / Audit：`audit/eyeball-stats-and-restore-fix.md` |
| Plan B SB10（Phase 1.5 Gate mockup 04 — 4 states ok/warn/error/loading + red items always block + drill+sim 一致 + GATE_TIMEOUT/PARSE/API 三碼分流）| ✅ DONE merge `4f964fe` — sonnet 3 commits SB10 + opus cold review 5 GAP fix + 1 chore：(1) `gate-loading-wrap`/`gate-spinner`/`gate-loading-step` CSS 對齊 mockup verbatim；(2) 移除 dead 401 branch（apiFetch throws 不 returns）；(3) `EMPTY_HINT_VISIBLE_MS` constant + clearTimeout re-fire safe；(4) zh-TW 錯誤訊息 + e.message leak guard；(5) error code TIMEOUT/PARSE/API 分流。**Bonus**：fix-sonnet 抓到 7 處 `evaluate()` block `circlesPhase=1.5` 漏設 `view='circles'` pre-existing test bug `c4332b3`。Two-stage code review × 2 round APPROVED。jest 143/143 / Playwright `circles-gate` 14/14 / 9 PNG director Read（mobile-360/desktop-1280/iPad × ok/warn/error/loading）/ `audit/eyeball-mockup-04.md` |
| Plan C SB2（NSM Step 2 + Step 3 mockup 07 — sub-tabs + NSM 定義 3 fields + 4-dim 動態 labels per product type）| ✅ DONE merge `5bf8677` — sonnet 3 commits SB2 + opus cold review 5 critical + 8 important GAP fix + 1 chore：(1) **functional bug：[data-nsm-submit] 沒 click handler — Step 2→Gate / Step 3→Eval 全 dead，sonnet wire `POST :id/gate` body `{nsm,rationale}`（rationale = explanation+'\n\n'+businessLink）+ `POST :id/evaluate` body `{userNsm,userBreakdown}`**；(2) 16 dim cells `desc/coachQ/hint` verbatim from mockup（SaaS 4/4 drift 修正）；(3) RT toolbar 4 buttons (B/list/indent/outdent) + `.nsm-rt-tbtn` 全域 handler；(4) Step 3 submit copy「送出，取得 AI 評分」；(5) verbatim test asserts `.toEqual([...])` for attention + saas desc；(6) hint content 真內容 from `dim.hint` + label/aria-expanded toggle。**Bonus**：fix-sonnet 抓到 Playwright `(guest-)?` regex syntax 不是合法 glob 的 pre-existing bug — 改 `mockApis()` helper。Two-stage code review × 2 round APPROVED。jest 143/143 / Playwright `nsm-step-2-3` 14/14 / 7 PNG director cross-viewport Read（Desktop-1280/Mobile-360/iPad × step2-empty/filled/step3-attention/saas — **核心驗證 SaaS labels 啟用/席次/黏著/擴張 vs Attention 觸及/互動/習慣/留存 dynamic 切換正確**）+ 32 PNG post-fix 重拍 / `audit/eyeball-mockup-07.md` |
| Plan D SB2（Onboarding mockup 10 — welcome card + 4-step coachmark tour + dual-ring spotlight + localStorage `circles_onboarding_done`）| ✅ DONE merge `32dda21` — sonnet 2 commits SB2 + 1 chore：welcome card hand-waving + 開始引導 / 直接自己選題 + 4-step tour（練習模式/選擇題型/挑一道題目/開始練習）。`.onb-targeted` dual-ring（2px white + 6px navy + 9999px page-dim）— pixel-exact match mockup line 189-192。Mobile 採 desktop 同 pattern（full-page dim + floating tooltip near target，**NOT** sticky-bottom）。Step 4 spotlight 圍**整張 expanded q-card**（含 desc + CTA）。Two-stage code review APPROVED。jest 143/143 / Playwright `onboarding` 11/11 / 8 PNG director cross-viewport Read（welcome/step1-4 × Desktop-1280/Mobile-360/iPad）/ `audit/eyeball-mockup-10.md`。**2 🟡 follow-up（非 blocking）**：step 3 selector `.qcard` → `.q-list`（mockup intent）/ step 4 fallback `.qcard:first-child` 缺 auto-expand。 |
| Combo C — Adversarial Input Quality + Frontend minLength（user 親抓 P0 bug：Phase 1 form 4 欄全 A → AI hallucinate 1 ok）| ✅ DONE 10 commits `1c97657..6d217fe` — User 親准 Path 2 carve-out 動 5 prompts。**3-Layer 守門**：(L1) Frontend minLength — `fieldMinLengthOk(value, floor)` helper，Phase 1 C1/I/R/C2 4-field 用 minMax 下限；NSM Step 2 (10/30/30) + Step 3 (20×4)；char-counter 加 `.is-below-floor` warn-color + 「（至少 N 字）」suffix；inline tip on disabled-button click。(L2) 5 prompts 加 `## 輸入品質檢查` 段（< 10 字 / 重複字元 / whitespace / unicode / 離題 / injection / 多欄相同 → error；嚴禁 hallucinate ok/warn）。(L3) `tests/adversarial/` 真 OpenAI sweep — 5 stages × 10 cases = **50/50 GREEN**。Frontend Playwright +13 specs GREEN。jest 143/143 baseline 不破。Total dev-cycle OpenAI cost ~\$1.25-1.50（under \$2.50 ceiling）。npm script `test:adversarial`（opt-in 不入 default CI）。**Standing rule 已入 memory**（`feedback_adversarial_review_testing.md`）：「所有階段審核都要經過極端情境測試」— 任何新 AI 審核 / gate / 評分 stage ship 前必跑 sweep。Spec：`docs/superpowers/specs/2026-05-08-adversarial-input-quality-design.md` / Plan：`docs/superpowers/plans/2026-05-08-adversarial-input-quality-combo-c.md` / Audit：`audit/adversarial-baseline-2026-05-08.md`。**已知非 blocking gap**：Phase 1 L/E/S step 結構（sol-cards / nested per-sol / 4 tracking）frontend 未守門，靠 Layer 2 prompt guard catch；future improvement candidate。 |
| Plan B Phase 2（mockup 05 全 6 sections — 對話練習 對話開頭 empty + icebreaker / 中段 3-bubble + turn counter + coach hint toggle / Streaming SSE + 3-dot bubble / 對話足夠 pill / 結論 box + 確認提交 + 繼續對話 / 已評分 locked banner + 2-button row）| ✅ DONE 7 commits — SB-A `3901901..822f79b` (4 commits) + SB-B `4786f3a..5710754` (3 commits)：sonnet implement + opus director cold review APPROVED 兩階段。SB-A：AppState 6 new fields + Phase 2 router + renderCirclesPhase2 Sections A+B (LOCKED helpers reuse + 7-step icebreaker copy table + renderChatBubble 3 variants + turn-counter chip)。SB-B：streamCirclesMessage SSE via fetch+getReader (auth/guest 雙 endpoint) + .conclude-pill turns≥3 trigger + renderConclusionBox + .conclusion-actions + .chat-body--dimmed 45% + 確認提交 → POST conclusion-check → POST evaluate-step → phase=3 + Section F locked banner + 「上一步看框架/回評分」+ Layer 1 minLength (userMessage ≥5 / conclusionText ≥30) + localStorage draft cache。**Pre-implementation 18-PNG audit**：opus 親 Read mockup 05 全 6 sections × 3 vp = 18 frames 建立視覺契約 → spec + plan in commit `05f1fe9`。**全 8-vp 親跑驗證**：phase2-chat 224/224 (28 specs × 8 vp 37 s) + critical regression 464/464 × 8 vp 1.4 min + jest 143/143 baseline 不破。**Opus director cold-Read 12 production PNGs**：streaming bubble + dim chat + locked banner 全對齊 mockup。**3 🟡 non-blocking drifts**：A-1 qchip suffix tablet (Phase 1 inherited) / E-1 eyebrow uppercase 一致 design pattern / F-1 progress bar fixture limit。docs：`audit/eyeball-mockup-05.md` + `audit/png-prod-mockup-05/` (18 PNG)。 |
| Plan B Phase 3 Score（mockup 11 全 4 sections — 預設 78 分 + 4 dim 折疊/展開 + 最強表現/最需改進 + 教練示範 accordion / 低分 52 + 邏輯性 1/5 自動展開 + 教練示範 multi-section context+perField+reasoning / Loading 56px spinner + 4-step checklist + 30s timeout / Error cloud-warning + EVAL_TIMEOUT badge + 重新評分 retry）| ✅ DONE 4 commits `bf42fda..b819b2c` — sonnet implement + opus director cold review APPROVED：(P3-1) AppState 4 fields + Phase 3 view router；(P3-2+P3-3) Section A+B score-card Instrument Serif italic + .dim-row 4 變體 (collapsed mobile / auto-expand desktop / score≤2 force-open) + .highlights 2-col + .coach-demo accordion + multi-section coach-demo body (context + perField iter 4 sub-blocks + reasoning) auto-open 低分；(P3-4) Section C Loading 56px circular spinner + .score-loading-step 4 變體 + setInterval 5s 推進 + 30s setTimeout 兜底 + Section D Error 80px danger cloud-warning + .error-code-badge mono + 返回修改答案/重新評分 retry wire；(P3-5) 12 production PNG audit + iOS 15-item + eyeball doc。**Pre-implementation 12-PNG audit**：opus 親 Read mockup 11 全 4 sections × 3 vp 建立視覺契約 → spec + plan in commit `574a764`。**全 8-vp 親跑驗證**：phase3-score 272/272 (34 specs × 8 vp 42.7 s) + critical regression 86/86 Desktop-1280 + jest 143/143。**Opus director cold-Read 4 production PNGs**：A-mobile 78 italic + 4 dim + highlights / B-desktop 52 + 邏輯性 1/5 auto-expand + coach-demo multi-section / C-tablet spinner + 4-step / D-desktop EVAL_TIMEOUT badge — 全對齊 mockup 0 drift。**3 🟡 non-blocking**：A-mobile coach-demo 需 scroll、`_phase3CoachDemoInitialized` side-effect、window-scope timer（已用 clearPhase3Timers 兜底）。docs：`audit/eyeball-mockup-11.md` + `audit/png-prod-mockup-11/` (12 PNG)。 |
