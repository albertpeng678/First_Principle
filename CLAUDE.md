...
> **Last updated:** 2026-05-22 — **P0-SCHEMA-1-v2 SHIPPED 2 commits `57600d5` (BE) + `7ff9a49` (FE)**

## 🟢 SHIPPED — P0-SCHEMA-1-v2 (NSM /evaluate shape coerce) 2026-05-22
- 2-commit ship: BE coerce defense `57600d5` + FE source fix `7ff9a49`
- 4 actual BE DB write sites (sonnet 抓到 spec over-scope: spec 寫 6，actual = 4 — lines 131/108 是 JS function call 不是 DB write)
- Verify: API spec 30/30 + E2E 15/15 + jest 606/624 + nsm-full-flow 8/8
- 真實 server log 抓到 `[coerce-user-nsm] string→object` 在 real e2e flow 觸發 — 證明 latent bug 存在 + defense 生效
- 15 PNG 證據在 `audit/schema-1-v2-roundtrip/`; Director cold-Read sample 4/15 APPROVED
- 3-round opus quiz R1 BLOCKED → R2/R3 APPROVED_WITH_NITS
- 2-stage review per commit: BE spec APPROVED + quality APPROVED_WITH_NITS / FE combined APPROVED
- Tracker §1 entry moved to §5 closure
- **下一步**: SCHEMA-3 NSM guest_id index (1-line migration, safest next P0)
